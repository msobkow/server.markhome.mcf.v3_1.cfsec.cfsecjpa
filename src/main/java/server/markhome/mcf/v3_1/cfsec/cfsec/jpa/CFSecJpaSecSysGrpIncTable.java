...
public class CFSecJpaSecSysGrpIncTable implements ICFSecSecSysGrpIncTable
{
		...
		ICFSecSecSysGrpInc rec )
	{
		if (rec == null) {
			...
			CFSecJpaSecSysGrpInc created = schema.getJpaHooksSchema().getSecSysGrpIncService().create(jparec);
			return( created );
		}
		else {
		...
		ICFSecSecSysGrpInc rec )
	{
		if (rec == null) {
			...
			CFSecJpaSecSysGrpInc jparec = (CFSecJpaSecSysGrpInc)rec;
			CFSecJpaSecSysGrpInc updated = schema.getJpaHooksSchema().getSecSysGrpIncService().update(jparec);
			return( updated );
		}
		else {
	...
	{
		return( schema.getJpaHooksSchema().getSecSysGrpIncService().find(PKey) );
	}
	...
	public ICFSecSecSysGrpInc lockDerived( ICFSecAuthorization Authorization,
		ICFSecSecSysGrpIncPKey PKey )
	{
		return( schema.getJpaHooksSchema().getSecSysGrpIncService().lockByIdIdx(PKey) );
	}
	...
	public ICFSecSecSysGrpInc[] readAllDerived( ICFSecAuthorization Authorization ) {
		List<CFSecJpaSecSysGrpInc> results = schema.getJpaHooksSchema().getSecSysGrpIncService().findAll();
	...
	public ICFSecSecSysGrpInc[] readDerivedBySysGrpIdx( ICFSecAuthorization Authorization,
		CFLibDbKeyHash256 argSecSysGrpId )
	{
		List<CFSecJpaSecSysGrpInc> results = schema.getJpaHooksSchema().getSecSysGrpIncService().findBySysGrpIdx(argSecSysGrpId);
		ICFSecSecSysGrpInc[] retset = new ICFSecSecSysGrpInc[results.size()];
		int idx = 0;
		for (CFSecJpaSecSysGrpInc cur: results) {
			retset[idx++] = cur;
